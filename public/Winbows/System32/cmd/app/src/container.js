...
if (process.env.pipe) {
    ...
    const command = process.env.command || process.args.c || process.args.k;
    if (!command) throw new Error('cmd container requires a command to execute.');
    const evaluation = await shell.execCommand(command);
    if (evaluation?.type !== 'cli' || !evaluation.data?.process) {
        // Built-ins and GUI apps have no terminal streams to bridge. The
        // command has already been started by ShellInstance; only CLI WRTs
        // need this container's keyboard and stdout wiring.
        wrt = completedCommand;
        if (process.env.closeAfterCommand === '1') process.exit();
    } else {
        wrt = evaluation.data;
        browserWindow.changeTitle(wrt.__filename);
    }
...
wrt.process.on('exit', () => {
    ...
    if (process.env.closeAfterCommand === '1' || process.args.c) {
        process.exit();
    }
})
process.on('exit', () => {
