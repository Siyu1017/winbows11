...
function check() {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const photoBlobType = mineTypeToExtension(blob ? blob.type : '');
            const photoExtension = photoBlobType || path.extname(photoPath);
...
const buffer = await fs.readFile(photoPath);
const photoBlob = new Blob([buffer]);
const photoBlobType = mineTypeToExtension(photoBlob ? photoBlob.type : '');
const photoExtension = photoBlobType || path.extname(photoPath);
...
viewerCanvas.addEventListener('wheel', function (e) {
    ...
    var canvasPosition = getPosition(viewerCanvas);
