...
fs.getFileURL('./app.ico').then(url => browserWindow.changeIcon(url));
...
function getMimeType(filePath) {
    const extension = fsUtils.extname(filePath).toLowerCase();
    return ({
...
async function updatePage(e) {
    const path = e.path.includes('?') ? e.path.slice(0, e.path.indexOf('?')) : e.path;
...
async function createFileItem(parent, details, path) {
    ...
    item.addEventListener('contextmenu', async (e) => {
        ...
        } else if (details.type.search('javascript') > -1 || ['.wrt'].includes(fsUtils.extname(path))) {
            items.push({
...
async function localPageCrafter(path) {
    ...
    const res = (await fs.readdir(path)).filter(name => name !== '.winbows-legacy-migration-v1');
    const pageStat = await fs.stat(path);
    let dirs = [];
    let files = [];
    let stats = {};

    for (const name of res) {
        const itemPath = fsUtils.resolve( path, name);
    ...
    footerPageSize.innerHTML = formatBytes(pageStat.size || 0);
