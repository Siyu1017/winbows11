const fsUtils = await requireAsync('node:path');

async function isLocalFile(page) {
    const drive = /^([A-Za-z]):\//.exec(page);
    if (!drive) return false;
    return (await fs.enumerateVolumes()).some(volume => volume.id === drive[1].toUpperCase());
}
...
">A great number of sites will refuse to render due to their X-Frame-Options policy, or they may be using a framekiller. This policy is applied to most significant sites such as Google, so do not expect those to work.</div>
        ...
        try {
            ...
            if (stat.isDirectory()) {
                ...
                for (let index = 0; index < items.length; index++) {
                    const item = path.resolve(page, items[index]);
                    items[index] = item;
    ...
    async function updatePage(e) {
        ...
        const isLocalFileURL = await isLocalFile(path);
