...
console.log(`winbows11 v${__VERSION__}`);

// Hardware-manager client. The Service Worker owns the actual hardware state;
// this thin API deliberately tolerates the first page load, where no controller
// exists yet.
if ('serviceWorker' in navigator) {
    const listeners = new Map();
    const hardwareState = { NIC: true };
    let registration;

    const emit = (eventName, value) => {
        for (const listener of listeners.get(eventName) || []) {
            try { listener(value); } catch (error) { console.error(`[HMGR] listener failed for ${eventName}`, error); }
        }
    };

    const send = async (message) => {
        const worker = navigator.serviceWorker.controller || registration?.active || await navigator.serviceWorker.ready.then(reg => reg.active);
    ...
    window.HMGR = {
        get state() { return { ...hardwareState }; },
        on(eventName, callback) {
            if (typeof callback !== 'function') throw new TypeError('HMGR listener must be a function');
            if (!listeners.has(eventName)) listeners.set(eventName, new Set());
            listeners.get(eventName).add(callback);
            return () => window.HMGR.off(eventName, callback);
        },
        off(eventName, callback) {
            listeners.get(eventName)?.delete(callback);
        },
        emit,
        async setHardware(name, enabled) {
            if (typeof name !== 'string') throw new TypeError('Hardware name must be a string');
            return send({ type: 'HMGR:SET_HARDWARE', value: { hardware: name.toUpperCase(), enabled: Boolean(enabled) } });
        },
        enable(name) { return this.setHardware(name, true); },
        disable(name) { return this.setHardware(name, false); },
        getState() { return send({ type: 'HMGR:GET_STATE' }); }
    };

    navigator.serviceWorker.addEventListener('message', event => {
        const message = event.data;
        if (!message || typeof message.type !== 'string') return;
        if (message.type === 'HMGR:STATE' && message.value && typeof message.value === 'object') {
            Object.assign(hardwareState, message.value);
        }
        emit(message.type, message.value);
    });

    navigator.serviceWorker.register('/hmgr.js', { updateViaCache: 'none', scope: './' })
        .then(async reg => {
            registration = reg;
            // The taskbar does not currently persist Wi-Fi state. Reset NIC on a
            // new page load so an old worker state cannot trap the user offline.
            await window.HMGR.setHardware('NIC', true);
            await window.HMGR.getState();
        })
        .catch(error => console.error('[HMGR] registration failed:', error));
}
...
!(async function Start() {
    ...
    if (!navigator.onLine) {
        ...
        try {
            const req = await fetch(`/assets/scripts/kernel.js?timestamp=${new Date().getTime()}`);
        ...
        if (localBuildId != __BUILD_ID__) {
            // needs to update
            window.needsUpdate = true;

            if (getJsonFromURL()['bypass-installation']) {
                localStorage.setItem('WINBOWS_BUILD_ID', __BUILD_ID__);
                location.reload();
                return;
            }
        }

        if (localBuildId != __BUILD_ID__ && !getJsonFromURL()['skip-installation']) {
