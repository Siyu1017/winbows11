...
export class BrowserWindow extends EventEmitter {
    constructor(config = {}) {
        ...
        this.x = (config.x == 'center' || !config.x) ? viewport.width / 2 - this.width / 2 : config.x;
        this.y = (config.y == 'center' || !config.y) ? viewport.height / 2 - this.height / 2 : config.y;
        ...
        this.animationData = {
            from: {
                x: this.x,
                y: this.y,
                scaleX: this.width / 800,
                scaleY: this.height / 600,
                opacity: 0,
                ts: Date.now()
            },
            to: {
...
${this.animationData.from.scaleY + (this.animationData.to.scaleY - this.animationData.from.scaleY) * p}
    ...
    handleMoveMoving = (e) => {
        if (this.pointerDown) {
            ...
            if (this.originalSnapSide != '' || this.isMaximized == true || this.browserWindow.window.offsetWidth != this.width) {
